...
このトピックは、コード例を示して、ラジアル エリア シリーズを link:{DataChartLink}.{DataChartName}.html[{DataChartName}] コントロールで使用する方法を説明します。

== 概要

ラジアル エリア シリーズは、データ ポイントを結ぶ直線のコレクションによってバインドされた塗りつぶされた多角形の形をしています。link:{DataChartLink}.radialareaseries.html[RadialAreaSeries] は link:{DataChartLink}.areaseries.html[AreaSeries] と同じデータ プロットの概念を使用していますが、横の線に沿って伸びるのではなく、円の周りでデータ ポイントをラップします。このシリーズのグループの詳細については、 link:datachart-radial-series-overview.html[ラジアル シリーズ]トピックを参照してください。
...
図 1: RadialAreaSeries の実装例

== データ バインディング

以下のコードは、サンプルのラジアル データ (link:resources-sample-category-data.html[サンプル カテゴリ データ] リソースからダウンロードで入手可能) を RadialAreaSeries にバインドする方法を示します。この例は {DataChartName} コントロール用のプロジェクトを設定済みであることを前提としています。
