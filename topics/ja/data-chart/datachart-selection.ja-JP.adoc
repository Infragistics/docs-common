...
== 選択の設定
デフォルトの動作 link:{DataChartLink}.{DataChartName}{ApiProp}SelectionMode.html[SelectionMode] はオフになっており、次のいずれかのオプションを選択する必要があります。link:{DataChartLink}.{DataChartName}.html[{DataChartName}] では、いくつかの選択モードを使用できます:
...
併せて、どの項目を選択するかをより細かく制御できる link:{DataChartLink}.{DataChartName}{ApiProp}SelectionBehavior.html[SelectionBehavior] も利用できます。`Auto` のデフォルトの動作は `PerSeriesAndDataItemMultiSelect` です。
...
その他の選択モードでは、さまざまな選択方法が提供されます。たとえば、PerDataItemMultiSelect とともに link:{DataChartLink}.{DataChartName}{ApiProp}SelectionBehavior.html[SelectionBehavior] を使用すると、複数のシリーズが存在する場合にカテゴリ全体のすべてのシリーズが選択されますが、一度に選択できるカテゴリは 1 つだけであり、以前に選択されていたものはすべて選択解除されます。
...
== アウトライン選択の構成
`FocusBrush` は、選択モードとともにシリーズのアウトラインのスタイルを設定するために適用できます。link:{DataChartLink}.{DataChartName}{ApiProp}SelectionMode.html[SelectionMode] プロパティがフォーカス オプションの 1 つに設定されている場合に、選択されたシリーズが境界線付きで表示されます。
