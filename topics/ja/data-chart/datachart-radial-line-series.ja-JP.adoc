...
このトピックは、コード例を示して、ラジアル折れ線シリーズを link:{DataChartLink}.{DataChartName}.html[{DataChartName}] コントロールで使用する方法を説明します。

== 概要

ラジアル折れ線シリーズは、データ ポイントを結ぶ直線のコレクションを使用して描画されます。link:{DataChartLink}.radiallineseries.html[RadialLineSeries] は link:{DataChartLink}.lineseries.html[LineSeries] と同じデータ プロットの概念を使用していますが、横の線に沿って伸びるのではなく、円の周りでデータ ポイントをラップします。このシリーズのグループの詳細については、 link:datachart-radial-series-overview.html[ラジアル シリーズ] トピックを参照してください。

== プレビュー

図 1 は、{DataChartName} コントロールにプロットされた時にラジアル折れ線シリーズがどのように表示されるのかを示します。

image::images/xamDataChart_Radial_Series_03.png[]

図 1: RadialLineSeries の実装例

== データ バインディング

以下のコードは、サンプルのラジアル データ (link:resources-sample-category-data.html[サンプル カテゴリ データ]リソースからダウンロードで入手可能) を RadialLineSeries にバインドする方法を示します。この例は {DataChartName} コントロール用のプロジェクトを設定済みであることを前提としています。
...
<ig:{DataChartName} x:Name="DataChart">
    ...
    <ig:{DataChartName}.Series>
        <ig:RadialLineSeries AngleAxis="{x:Reference angleAxis}"
...
var series = new RadialLineSeries();
