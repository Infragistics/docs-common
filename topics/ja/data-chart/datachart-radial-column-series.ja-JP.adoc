...
このトピックは、コード例を示して、ラジアル柱状シリーズを link:{DataChartLink}.{DataChartName}.html[{DataChartName}] コントロールで使用する方法を説明します。

== 概要

ラジアル柱状シリーズは、チャートの中心からデータ ポイントの位置に向けて広がる矩形のコレクションを使用して描画されます。link:{DataChartLink}.radialcolumnseries.html[RadialColumnSeries] は link:{DataChartLink}.columnseries.html[ColumnSeries] と同じデータ プロットの概念を使用していますが、横の線に沿って伸びるのではなく、円の周りでデータ ポイントをラップします。このシリーズのグループの詳細については、 link:datachart-radial-series-overview.html[ラジアル シリーズ]トピックを参照してください。

== プレビュー

図 1 は、{DataChartName} コントロールにプロットされた時にラジアル柱状シリーズがどのように表示されるのかを示します。
...
<ig:{DataChartName} x:Name="DataChart">
    ...
    <ig:{DataChartName}.Series>
        <ig:RadialColumnSeries AngleAxis="{x:Reference angleAxis}"
