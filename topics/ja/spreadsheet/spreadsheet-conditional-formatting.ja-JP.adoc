...
このトピックは、link:{SpreadsheetLink}.{SpreadsheetName}.html[{SpreadsheetName}] コントロールで link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet.html[Worksheet] で条件付き書式を構成して設定する方法を説明します。
...
|このトピックでは、`{SpreadsheetName}` を短時間で起動、実行するために役立つ詳細な操作方法を紹介します。

|link:spreadsheet-conf-worksheets.html[ワークシート]
|このトピックでは、`{SpreadsheetName}` でワークシートを構成する方法を説明します。
...
既存の Workbook を Excel から読み込む場合、その Workbook が `{SpreadsheetName}` に読み込まれるときに書式設定が保持されます。Workbook を Excel ファイルに保存する場合にも保持されます。
...
`Worksheet` 要素にデータを読み込んだ後、結果は以下のような {SpreadsheetName} になります。
...
* link:spreadsheet-working.html[{SpreadsheetName} の作業]
