...
{SpreadsheetName} コントロールは、 link:{SpreadsheetLink}.spreadsheetcellrangeformat_members.html[SpreadsheetCellRangeFormat] 型の link:{SpreadsheetLink}.{SpreadsheetName}{ApiProp}activeselectioncellrangeformat.html[ActiveSelectionCellRangeFormat] という名前の特別なプロパティを提供します。これは、アクティブな選択の異なる書式要素 (フォント プロパティ、配置、インデントなど) の設定または取得に使用できます。
