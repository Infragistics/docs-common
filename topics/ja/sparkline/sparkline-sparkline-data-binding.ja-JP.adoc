...
最低要件として _{SparklineName}_ では、コントロールをデータ コレクションのインスタンスにバインドする pick:[wpf,win-universal,xamarin="link:{SparklineLink}.{SparklineName}{ApiProp}itemssource.html[ItemSource]"]pick:[win-forms="link:{SparklineLink}.{SparklineName}{ApiProp}datasource.html[DataSource]"] プロパティと、どのデータを表示するか特定する link:{SparklineLink}.{SparklineName}{ApiProp}valuememberpath.html[ValueMemberPath] プロパティが必要です。
