...
近似曲線機能により、トレンド性向ラインを生成する複数の数式を選択できます。数式は link:{SparklineLink}.{SparklineName}~trendlinetype.html[TrendLineType] プロパティで指定します。シリーズ データがいつでも見えるよう、近似曲線はそのデータの前に描画されます。
...
X 軸と Y 軸の可視性は個別のプロパティ (それぞれ link:{SparklineLink}.{SparklineName}~horizontalaxisvisibility.html[HorizontalAxisVisibility] と link:{SparklineLink}.{SparklineName}~verticalaxisvisibility.html[VerticalAxisVisibility]) で管理されるため、その 1 つだけ表示させることができます。
