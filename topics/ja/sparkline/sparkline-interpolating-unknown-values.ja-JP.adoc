...
不明な値の補間機能は、 link:{SparklineLink}.{SparklineName}~unknownvalueplotting.html[UnknownValuePlotting] プロパティにより有効になり、(補間のタイプという意味で) 設定されます (<<_Ref317177339,プロパティ設定>>のブロックを参照)。
...
以下の表は、{SparklineName} 補間オプションとそれらを提供する link:{SparklineLink}.{SparklineName}~unknownvalueplotting.html[UnknownValuePlotting] プロパティ設定の関係を示しています。

[options="header", cols="a,a"]
|====
|補間オプション| link:{SparklineLink}.{SparklineName}~unknownvalueplotting.html[UnknownValuePlotting] プロパティ設定
...
以下のスクリーンショットは、 link:{SparklineLink}.{SparklineName}~unknownvalueplotting.html[UnknownValuePlotting] プロパティのさまざまな設定を使用して、同じデータ セットから描画されたスパークラインを示しています。
