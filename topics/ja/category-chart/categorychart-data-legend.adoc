...
link:{DataChartLink}.{DataLegendName}.html[{DataLegendName}] は `Legend` のように機能するコンポーネントですが、シリーズの値の表示や、シリーズの行と値の列のフィルタリング、値のスタイルと書式を設定するための多くの構成プロパティを提供します。この凡例は、link:{DataChartLink}.{CategoryChartName}.html[{CategoryChartName}] のプロット領域内でマウスを移動すると更新され、ユーザーのマウス ポインターがプロット領域を離れたときに最後にホバーされたポイントを記憶する永続的な状態になります。このコンテンツは、3 種類の行と 4 種類の列のセットを使用して表示されます。
...
単位の列には、省略記号や単位テキストが表示されます。これらは、link:{DataChartLink}.{DataLegendName}~UnitsText.html[UnitsText] プロパティを設定することで link:{DataChartLink}.{DataLegendName}.html[{DataLegendName}] に設定できます。

次のコードスニペットは、link:{DataChartLink}.{DataLegendName}~UnitsText.html[UnitsText] と link:{DataChartLink}.{DataLegendName}.html[{DataLegendName}] に設定された最小/最大の端数を示しています。
...
以下のコード スニペットは、上記の行のスタイル設定イベントの使用法を示しています。
