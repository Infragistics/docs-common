...
このトピックでは、立体円柱棒グラフでデータが適切に描画されることを保証する支援となる役に立つ情報を説明します。
...
以下は、立体円柱棒グラフのデータ要件のリストです。

* データ セットには、ひとつまたは複数の数値列が含まれます。
* 立体円柱棒グラフを Series オブジェクトにバインドする場合には、Series バインディング要件の詳細は link:chart-requirements-for-series-binding.html[「Series バインディングの要件」]を参照してください。
...
描画された立体円柱棒グラフとともに、サンプルのデータ セットは以下を参照してください。
