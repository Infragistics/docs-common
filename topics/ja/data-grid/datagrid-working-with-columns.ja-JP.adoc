...
これを前提とすると、次に示すコードのように、このリストは {DataGridName} コントロールの link:{DataGridLink}.{DataGridName}{ApiProp}{ApiDataSource}.html[{ApiDataSource}] として割り当てられ、{DataGridName} コントロールの link:{DataGridLink}.{DataGridName}{ApiProp}autogeneratecolumns.html[pick:[android="set"]AutoGenerateColumn] {ApiMember}には、パラメーターとしてブール値 True が渡されます。
