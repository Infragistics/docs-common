...
このトピックは、link:{DataGridLink}.{DataGridName}.html[{DataGridName}]™ コントロールで link:ODataVirtualDataSource.html[ODataVirtualDataSource] を使用するために必要な `ODataVirtualDataProvider` の実装についての情報を提供します。
