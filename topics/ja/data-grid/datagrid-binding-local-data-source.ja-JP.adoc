...
[start=2]
. プロジェクト メイン ビューの `{DataGridName}` コントロールのインスタンスを作成し、link:{DataGridLink}.{DataGridName}{ApiProp}{ApiDataSource}.html[{ApiDataSource}] {ApiMember}を使用して、次のサンプル コードに示すようにサンプル データを生成します。
