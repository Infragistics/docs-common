...
This topic provides information on how to display geographic imagery from ArcGIS Online servers in the background content of the link:{GeoMapLink}.xamgeographicmap_members.html[{GeoMapName}]™ control.
...
|This topic provides information about the layout of the visual elements in the _{GeoMapName}_ control
...
The _{GeoMapName}_ control displays geographic imagery from ArcGIS Online in the map background content using the link:{GeoMapLink}.arcgisonlinemapimagery_members.html[ArcGISOnlineMapImagery] class.

Public maps can be displayed by setting the link:{GeoMapLink}.arcgisonlinemapimagery~mapserveruri.html[MapServerUri] property of the link:{GeoMapLink}.arcgisonlinemapimagery_members.html[ArcGISOnlineMapImagery] class to the map’s specified URI.

Private maps involve more configuration than simply setting the link:{GeoMapLink}.arcgisonlinemapimagery~mapserveruri.html[MapServerUri]. You must use an access token either provided by the owner of the map or generated via a Username, and Password combination also provided by the owner.

[[Preview]]
=== Preview

The following screenshot is a preview of the _{GeoMapName}_ control with geographic imagery tile from map content on an ArcGIS Online server.
...
In order to use geographic imagery from ArcGIS public maps, you must first obtain the map’s public URI. You must then set this map URI to the link:{GeoMapLink}.arcgisonlinemapimagery_members.html[ArcGISOnlineMapImagery] object’s link:{GeoMapLink}.arcgisonlinemapimagery~mapserveruri.html[MapServerUri] property.

[[Example1]]

=== Example

The following screenshot illustrates the rendering of the  _{GeoMapName}_  control with the link:{GeoMapLink}.xamgeographicmap~backgroundcontent.html[BackgroundContent] property set to the link:{GeoMapLink}.arcgisonlinemapimagery_members.html[ArcGISOnlineMapImagery].

The link:{GeoMapLink}.arcgisonlinemapimagery~mapserveruri.html[MapServerUri] property of the link:{GeoMapLink}.arcgisonlinemapimagery_members.html[ArcGISOnlineMapImagery] object is set to following value:
...
The following table summarizes the credential properties of the link:{GeoMapLink}.arcgisonlinemapimagery_members.html[ArcGISOnlineMapImagery] object used for accessing the private map content on ArcGIS Online servers.
...
|String
|This optional property specifies the _Referer_ contained in the _WebRequest_. In the case of WPF a default value is set.
...
The following code example demonstrates how to set the link:{GeoMapLink}.arcgisonlinemapimagery_members.html[ArcGISOnlineMapImagery] object’s properties required for a private map.

.Note
[NOTE]
====
You must replace strings set to the link:{GeoMapLink}.arcgisonlinemapimagery~username.html[UserName], link:{GeoMapLink}.arcgisonlinemapimagery~password.html[Password], link:{GeoMapLink}.arcgisonlinemapimagery~mapserveruri.html[MapServerUri] and link:{GeoMapLink}.arcgisonlinemapimagery~refereruri.html[ReferURI] properties to your own credentials and map information.
====
