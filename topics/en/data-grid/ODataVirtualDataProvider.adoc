...
= ODataVirtualDataProvider
 
This topic provides implementation of `ODataVirtualDataProvider` that is required for using the link:ODataVirtualDataSource.html[ODataVirtualDataSource]  with the link:{DataGridLink}.{DataGridName}.html[{DataGridName}]™ control.
