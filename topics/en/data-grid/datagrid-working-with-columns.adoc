...
Continuing this thought process, imagine that the list were assigned as the link:{DataGridLink}.{DataGridName}~{ApiDataSource}.html[{ApiDataSource}] of the {DataGridName} control, while the link:{DataGridLink}.{DataGridName}{ApiProp}AutoGenerateColumns.html[pick:[android="set"]AutoGenerateColumn] {ApiMember} of the {DataGridName} control is passed the boolean value of true as its parameter, as demonstrated in the following code snippet.
