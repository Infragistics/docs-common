...
Legend items can be positioned in a vertical or table structure via the link:{DataChartLink}.{DataLegendName}{ApiProp}LayoutMode.html[LayoutMode] property. The default value is `Table`, which retains the same look and feel as seen in previous releases.
...
By default, DataLegend will hide names of groups, but you can display group names by setting the - link:{DataChartLink}.{DataLegendName}{ApiProp}GroupRowVisible.html[GroupRowVisible] property to true.
