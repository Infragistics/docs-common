...
The default behavior link:{DataChartLinkBase}.seriesviewer{ApiProp}SelectionMode.html[SelectionMode] turned off and requires opting into one of the following options. There are several selection modes available in the link:{DataChartLink}.{DataChartName}.html[{DataChartName}]:
...
In conjuction, a link:{DataChartLinkBase}.seriesviewer{ApiProp}SelectionBehavior.html[SelectionBehavior] is available to provide greater control on which items get selected. The default behavior for `Auto` is `PerSeriesAndDataItemMultiSelect`.
...
Other selection modes offer various methods of selection. For example using link:{DataChartLinkBase}.seriesviewer{ApiProp}SelectionBehavior.html[SelectionBehavior] with PerDataItemMultiSelect will select all series in entire category when multiple series are present, but only one category at a time, deselecting anything selected previously. 
...
A `FocusBrush` can be applied to style the outline of a series, along with the selection modes. Selected series will appear with a border when the link:{DataChartLinkBase}.seriesviewer{ApiProp}SelectionMode.html[SelectionMode] property is set to one of the focus options.
