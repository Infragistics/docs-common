...
= Adding {SparklineName} to a Page



=== Purpose

This topic demonstrates, with code examples, how to add the link:{ApiPlatform}{SparklineAssembly}.html[{SparklineName}]™ control to a page. If you want to display  _{SparklineName}_   inside grid cells, refer to the Sparkline Column topic.
...
* <<_Ref320004748,Code Example: Adding a {SparklineName} to a {PlatformName} application>>
...
== Adding a {SparklineName} to a Page – Conceptual Overview

=== Introduction

Though  _{SparklineName}_   is designed with the idea to be inserted in much small spaces such as grid cells or in-line with text, it has the capability to be displayed as a stand-alone control, e.g. in web pages.
...
The following table summarizes the requirements for adding the  _{SparklineName}_   control.

[cols="a,a"]
|====
|Assembly references
|See the following topic for more information, link:xamarin-project-with-infragistics-controls#NuGetPackages.html[Add references through NuGet Packages]
...
*Adding the {SparklineName} control*
...
*Adding the _{SparklineName}_ control*
...
Create a class definition to represent the data model for the Sparkline. At least two numeric fields is needed by  _{SparklineName}_   control in order to render the Sparkline.
...
The following code demonstrates adding a namespace and referencing the local classes. Specifying the key (`x``:Key`) in the resource allows binding the  _{SparklineName}_   to the underlying data as a static resource.
...
[start=4]
. Add the {SparklineName} control.

The following code demonstrates adding the  _{SparklineName}_   with the minimum code and property settings required for display.
