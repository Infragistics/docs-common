...
This topic explains how to configure and set conditional formatting on a link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet.html[Worksheet] in the link:{SpreadsheetLink}.{SpreadsheetName}.html[{SpreadsheetName}] control.
...
|link:spreadsheet-conf-worksheets.html[Worksheets]
|This topic explains how to configure the worksheets in the `{SpreadsheetName}`.
...
When loading a pre-existing Workbook from Excel, the formats will be preserved when that Workbook is loaded into the `{SpreadsheetName}`. The same is true for when you save the Workbook out to an Excel file.
...
After filling the `Worksheet` element with data, the above will result in a {SpreadsheetName} that looks like the following:

image:images/spreadsheet-conditional-formatting.png[]
