...
= Accessing Cells and Cell Regions by Name ({ExcelEngineName})

== Topic Overview

=== Purpose

This topic explains, with code examples, how to access link:excelengine-glossary-of-infragistics-excel-engine-terms.html[cells] and link:excelengine-glossary-of-infragistics-excel-engine-terms.html[regions] within a worksheet in a Microsoft® Excel® workbook by their names or by their named references. For information about how to access cells and regions by their reference strings, refer to the link:excelengine-reference-cells-and-regions.html[Referencing Cells and Regions] topic.
...
The {ProductName} product supports the referencing of cells and regions by Name through the link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet~getcell.html[GetCell] and link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet~getregion.html[GetRegion] methods of the link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet_members.html[Worksheet] object. You refer to the cell or region using the  _NamedReference_ instance that refers to that cell or region.
...
|Getting an link:excelengine-glossary-of-infragistics-excel-engine-terms.html[Array of Regions] by Name
|The `Worksheet` object’s method link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet~getregions.html[GetRegions] allows a named reference to be specified to access an array of regions.

|Getting a Referenced Cell, Region, and Array of Regions from a Named Reference
|A reference to a cell or region can be obtained using the link:{ExcelEngineApiLink}infragistics.documents.excel.namedreference_members.html[NamedReference] instance that refers to a cell or region.
...
|The {ExcelEngineName} allows you to add Microsoft® Excel® formulas to a cell or group of cells in a worksheet.

| link:excelengine-add-a-comment-to-a-cell.html[Adding a Comment to a Cell ({ExcelEngineName})]
|A comment allows you to display hints or notes for a cellpick:[wpf=" when the end user's mouse hovers over a cell"]. The comments display as a tooltip-like callout that contains text.
