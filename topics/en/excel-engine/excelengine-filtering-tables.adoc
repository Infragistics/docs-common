...
= Filtering Tables pick:[wpf,xamarin="({ExcelEngineName})"]
...
|Custom Filtering
|This code example shows how cells can be filtered according to one or more pick:[win-forms,win-forms-old,asp-net,aspnet-old="CustomFilterConditions"]pick:[wpf,xamarin="link:{ExcelEngineApiLink}infragistics.documents.excel.filtering.customfiltercondition.html[CustomFilterCondition]s"].
...
In this example code snippet for programmatically creating a workbook, a  _Patients_   named table is used. pick:[win-forms,win-forms-old,asp-net,aspnet-old="The code snippet can be found here: link:excelengine-creating-excel-named-table-code-example.html[Creating Excel Named Table Code Example]"]
