...
= Inserting and Deleting Cells, Column and Rows ({ExcelEngineName})
...
The {ExcelEngineName} supports inserting of new cell(s) and removing of existing cell(s) using methods located in the link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet_members.html[Worksheet] class.
...
The {ExcelEngineName} supports inserting of one or more new cell(s) by specifying the location at which the insert process should occur. The cell(s) already located at the specified address(es) will be shifted to the right or below. You can specify the address(es) for the new cell(s) using a `string` or a link:{ExcelEngineApiLink}infragistics.documents.excel.worksheetregion_members.html[WorksheetRegion].
...
The {ExcelEngineName} supports inserting and deleting column(s) using the `Insert` and `Remove` methods of the worksheet’s link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet~columns.html[Columns] collection.
...
The {ExcelEngineName} supports inserting and deleting row(s) using the `Insert` and `Remove` methods of the worksheet’s link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet~rows.html[Rows] collection.
...
| link:excelengine-accessing-cells-and-cell-regions-by-name.html[Accessing Cells and Cell Regions by Name ({ExcelEngineName})]
