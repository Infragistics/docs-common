...
* <<_Ref319876564,Code Example: Fill Sort Condition>>
...
This example uses a code snippet for programmatically creating a workbook. In the workbook a  _Patients_   named table is used. pick:[win-forms,win-forms-old,asp-net,aspnet-old="The code snippet can be found here: link:excelengine-creating-excel-named-table-code-example.html[Creating Excel Named Table Code Example]"]
...
|This topic describes table support in {ExcelEngineName}. A table in Microsoft Excel® is a named rectangular region of cells which contain related data organized into columns.
