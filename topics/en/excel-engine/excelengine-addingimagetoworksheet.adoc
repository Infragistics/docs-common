...
= Adding an Image to a Worksheet ({ExcelEngineName})

== Topic Overview

=== Purpose

This topic provides procedural instructions on how to add an image to a worksheet using the {ExcelEngineName}.
...
This procedure demonstrates how to add an image to a worksheet. Images are represented by the link:{ExcelEngineApiLink}infragistics.documents.excel.worksheetimage_members.html[WorksheetImage] class and are added in the link:{ExcelEngineApiLink}infragistics.documents.excel.worksheet~shapes.html[Shapes] collection of the worksheet.
...
You need to specify the bounds of the image in twips using the link:{ExcelEngineApiLink}infragistics.documents.excel.worksheetimage~setboundsintwips.html[SetBoundsInTwips] method.
