...
The units column displays an abbreviation symbol and/or unit text, which can be set either on the link:{DataChartLink}.{DataLegendName}.html[{DataLegendName}] by setting the link:{DataChartLink}.{DataLegendName}~UnitsText.html[UnitsText] property.

The following code snippet demonstrates the link:{DataChartLink}.{DataLegendName}~UnitsText.html[UnitsText] and the minimum/maximum fractions set on the link:{DataChartLink}.{DataLegendName}.html[{DataLegendName}]:
