...
--
+
* link:{BarcodesLink}.{BarcodeMaxiName}~postalcode.html[PostalCode] – the postal code/zip code of the delivery address. When in Mode 3, it should be up to 6 characters as only capital letters and digits are acceptable. When in structured mode 2 - up to 9 digits can be set.
* link:{BarcodesLink}.{BarcodeMaxiName}~serviceclass.html[ServiceClass] – a number in the range of 000 to 999 which is defined by the carrier and shipper to identify the service class.
* link:{BarcodesLink}.{BarcodeMaxiName}~country.html[Country] – a number in the range of 000 to 999 which represents a country. For a complete list of country codes, see ISO standard 3166.
